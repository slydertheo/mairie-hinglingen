...
import { useDocuments, useCouncil, useCommissions, useDeliberations, useAffichage } from '../../lib/contentStore';
...
export default function VieMunicipaleV2() {
  ...
  const councilMembers = useCouncil();
  const commissions = useCommissions();
  const deliberations = useDeliberations();
  const affichage = useAffichage();
  const bulletins = documents.filter(d => d.category === 'Bulletin');
  const comptes = documents.filter(d => d.category === 'Compte rendu');
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6">
                ...
                <div key={c.id} className="border border-gray-200 rounded-xl overflow-hidden">
                  ...
                  >
                    <div>
                      ...
                      <span className="text-gray-400 text-xs ml-2">— Présidée par {c.president}</span>
                    </div>
                  ...
                  {openComm === i && (
                    <div className="border-t border-gray-100 px-4 py-3 bg-gray-50">
                      <div className="flex flex-wrap gap-2">
                        <span className="text-xs bg-blue-100 text-blue-700 px-3 py-1 rounded-full font-medium">{c.president} (Président·e)</span>
                        {c.members.split(',').map(m => m.trim()).filter(Boolean).map(m => <span key={m} className="text-xs bg-gray-100 text-gray-700 px-3 py-1 rounded-full">{m}</span>)}
                      </div>
                    </div>
                  )}
                </div>
        ...
        {active === 5 && (
          <div>
            ...
            <div className="space-y-3">
              {affichage.map((a) => (
                <div key={a.id} className="flex items-center gap-4 border border-gray-200 rounded-xl p-4">
